...
int led_main(void)   // 정의 
{
   ...
   while(1) 
   {
      shift_left_ledon();  // dnt11_state 0
      shift_right_ledon(); // dnt11_state 1
...
void flower_on(void)
{
   ...
   if (msec_count >= 100)
   {
      ...
      {
         i=PORTA=0x00;
         //dnt11_state_transition();   // dnt11_state를 천이 to shift_right_keep_ledon
      }
      else
      {
         PORTA |= 0x10 << i | 0x08 >> i++;
...
void shift_right_ledon(void)
{
   ...
   if (msec_count >= 30)
   {
      ...
      {
         i=PORTA=0x00;
         //dnt11_state_transition();   // dnt11_state를 천이 to shift_left_keep_ledon
      }
      else
      {
         PORTA = 0b10000000 >> i++;
...
void dnt11_state_transition(void)
{
   dnt11_state++;
   dnt11_state %= FUNC_NUMER;
}