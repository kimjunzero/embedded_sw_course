...
volatile int ultrasonic_timer_count = 0;
...
void init_timer2()
{
	TCCR2 |= (1 << CS22) | (1 << CS21) | (1 << CS20); // 1024 분주
	TIMSK |= (1 << TOIE2);  // 오버플로우 인터럽트 활성화
}

// 타이머 카운터 2.
ISR(TIMER2_OVF_vect)
{
	ultrasonic_timer_count++;
	if (ultrasonic_timer_count >= 6)  // 약 100ms마다 실행
	{
		ultrasonic_timer_count = 0;
