...
int main(void)
{
	int button0_dnt11_state=0;  // 초기 상태를 0으로 출발
	...
	while (1)
	{
		...
		{
			button0_dnt11_state++;
			button0_dnt11_state %= 8;
		}
		fp[button0_dnt11_state] ();
	}
#endif
    ...
    while (1)   // for(;;) 
    {
		...
		if (get_button(BUTTON0, BUTTON0PIN))
		{
			button0_dnt11_state = !button0_dnt11_state;  // 반전 0 <--> 1
			if (button0_dnt11_state)
				led_all_on();
			else led_all_off();
		}
    }
